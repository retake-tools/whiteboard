...
assert.match(i18nSource, /'skillComposer\.referenceBadgeReference': '参考'/);
...
assert.match(agentPreferencesSource, /const hasMediaMode = availableModes\.some/);
assert.match(agentPreferencesSource, /\{!workflowSelected && hasMediaMode \? \(/);
assert.match(agentPreferencesSource, /const effectiveOutputType = workflowSelected \? 'image'/);
assert.doesNotMatch(agentPreferencesSource, /availableModes\.length > 2/);
