...
import { createAgentRunForOperation, startAgentRun } from '../src/core/agentRuntime';
...
const [portSource, workspaceSource, composerSource, controllerSource, appServerSource] = await Promise.all([
...
rejectChangeProposal(snapshot, proposalTurn.proposal!.proposalId);
...
assert.equal(messagesForSession(recovered, created.session.agentSessionId).length, 6);
assert.equal(runtimeBindingForSession(recovered, created.session.agentSessionId)?.externalThreadId, 'thread_test_001');
assert.equal(proposalsForSession(recovered, created.session.agentSessionId)[0]?.status, 'rejected');
