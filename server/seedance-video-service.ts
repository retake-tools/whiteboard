  ...
  try {
    for (let index = 0; index < input.resultBlockIds.length; index += 1) {
      ...
      await updateProviderCall({
        ...
        callIndex: index,
        patch: { status: 'submitted', providerTaskId: created.id },
      });
    ...
    await finishUnresolvedProviderCalls({
      ...
      executionId: execution.executionId,
      status: 'failed',
      error: error instanceof Error ? error.message : 'Seedance video generation failed.',
    });
    await failExecution({
      projectId: execution.projectId,
      boardId: execution.boardId,
...
async function buildSeedanceContent(snapshot: BoardSnapshot, execution: ExecutionRecord): Promise<SeedanceContentItem[]> {
  const { firstFrame, lastFrame, references } = videoAdapterAssetInputs(execution);
