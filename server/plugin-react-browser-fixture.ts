...
async function writeReactPluginSource(sourceRoot: string): Promise<void> {
  ...
  await writeJson(path.join(sourceRoot, 'retake.plugin.json'), {
    contributions: [
      ...
      {
        contributionId: 'retake.contribution.react-browser-fixture-settings',
  ...
  await writeFile(
    ...
    [
      ...
      "import { createPluginTranslator, defineMessages, definePluginContribution, defineSettings, pluginThemeVariable } from '@retake/plugin-api';",
      '',
      ...
      '  };',
      '  settings: {',
      '    getSnapshot(settingsId: string): { values: Readonly<Record<string, unknown>> } | null;',
      '    subscribe(listener: () => void): () => void;',
      '    update(input: { settingsId: string; scope: "workspace"; values: Readonly<Record<string, unknown>> }): Promise<unknown>;',
      '  };',
      ...
      '    () => host.settings.getSnapshot("retake.contribution.react-browser-fixture-settings"),',
      '    () => host.settings.getSnapshot("retake.contribution.react-browser-fixture-settings"),',
      '  );',
      '  const translator = createPluginTranslator(fixtureMessages, environment.locale);',
      '  if (globalThis.retakeReactPluginBrowserFixture?.crashPanel) {',
      '    throw new Error("fixture panel crash");',
      '  }',
      '  return <section data-retake-plugin="react-browser-fixture" style={{ transitionDuration: pluginThemeVariable("motion.duration.fast") }}>',
      '    <strong>{translator.t("title")}</strong>',
      ...
      '    <button type="button" onClick={() => host.settings.update({',
      '      settingsId: "retake.contribution.react-browser-fixture-settings",',
      '      scope: "workspace",',
      '      values: { compact: !(settings?.values.compact ?? false) },',
      '    })}>Toggle compact fixture</button>',
      '    {importedAsset ? <img alt="Imported fixture" height="16" src={importedAsset.previewUrl} width="16" /> : null}',
      '  </section>;',
      '}',
      '',
      'const fixtureMessages = defineMessages({',
      '  title: { default: "Plugin fixture", locales: { "zh-CN": "插件夹具" } },',
      '});',
      '',
      'export const fixtureSettings = defineSettings({',
      '  apiVersion: 1,',
      '  fields: {',
      '    compact: { default: false, label: "Compact", scope: "workspace", type: "boolean" },',
      '  },',
      '  kind: "settings",',
      '  schemaVersion: 1,',
      '  settingsId: "retake.contribution.react-browser-fixture-settings",',
      ...
      '  subscribeReadSnapshot(listener: () => void): () => void;',
      '} }) {',
      '  const hostReact = globalThis.retakePluginHostExternalsV3.react;',
