...
import { videoAdapterAssetInputs } from '../src/core/domainVideoAdapterInput';
...
import {
  finishUnresolvedProviderCalls,
  updateProviderCall,
} from './local-store/provider-call-store';
...
export async function cancelDreaminaCliVideoGeneration(input: {
  ...
  await saveSnapshot(snapshot);
  await finishUnresolvedProviderCalls({
  ...
  try {
    for (let index = 0; index < input.resultBlockIds.length; index += 1) {
      ...
      await updateProviderCall({
        ...
        callIndex: index,
        patch: { status: 'submitted', providerTaskId: submitted.submitId },
      });
      ...
      await updateProviderCall({
        ...
        executionId: execution.executionId,
        callIndex: index,
        patch: { status: 'running', providerTaskId: submitted.submitId },
      });
      await assertExecutionStillRunning(execution);
      ...
      try {
        ...
        await updateProviderCall({
          ...
          patch: {
            status: 'succeeded',
            providerTaskId: submitted.submitId,
    ...
    await finishUnresolvedProviderCalls({
      ...
      executionId: execution.executionId,
      status: 'failed',
      error: error instanceof Error ? error.message : 'Dreamina CLI video generation failed.',
    });
    await failExecution({
      projectId: execution.projectId,
      boardId: execution.boardId,
...
async function buildDreaminaCommandInput(
  ...
  const {
    firstFrame: firstFrames,
    lastFrame: lastFrames,
    references,
  } = videoAdapterAssetInputs(execution);
