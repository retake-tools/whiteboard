...
const registeredPluginPath = path.join(homedir(), 'plugins', pluginName);

let repositoryRoot;
try {
  repositoryRoot = await realpath(registeredPluginPath);
  ...
  console.error(
    `Retake Whiteboard source is not registered at ${registeredPluginPath}. ` +
